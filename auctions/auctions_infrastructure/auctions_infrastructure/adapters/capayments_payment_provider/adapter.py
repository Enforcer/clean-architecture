...
from auctions.application.ports.payment_provider import (
    PaymentFailedError,
    PaymentProvider,
)
from auctions_infrastructure.adapters.capayments_payment_provider.requests import (
    ChargeRequest,
    Request,
)
...
class CaPaymentsPaymentProvider(PaymentProvider):
    ...
    def pay_for_won_auction(self, auction_id: AuctionId, bidder_id: BidderId, charge: Money) -> None:
        ...
        request = ChargeRequest(
            card_token=dao.get_bidders_card_token(bidder_id),
            currency=currency,
            amount=amount,
        )
